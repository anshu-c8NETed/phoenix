...
import { OrbBg, GlassCard, GlassInput, GlassTextarea, DS } from "./DesignSystem";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import CrisisSpine, { CrisisSpineHandle } from "./CrisisSpine";
import EmberFieldBackdrop from "./EmberFieldBackdrop";
...
function AnimatedChance({ before, after, onLockIn }: { before: number; after: number; onLockIn?: () => void }) {
  ...
  return (
    <div className="p-5 rounded-xl border mt-6 relative overflow-hidden"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 items-center">
        <div className="odds-cell-before">
...
function AgentsShowcase() {
  ...
  return (
    <section className="w-full py-12 md:py-16 relative" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {/* Heading + intro split across the wider column so the line
            length stays readable while the row uses the full width */}
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-6 lg:gap-10 items-end mb-10 md:mb-14">
          ...
          <p className="text-[12px] font-mono text-zinc-600 lg:text-right lg:pb-2">
            {activeIdx === null ? "Tap or hover any agent — each one feeds the next." : "→ Output flows straight into the next agent's input."}
          </p>
        </div>
      </div>

      {/* Rows go full-bleed: the hover glow, scanline and connector
          spine now span the entire viewport width, while the row's
          inner content stays aligned to the same max-w-7xl measure
          as the heading above it via matching horizontal padding. */}
      <div className="flex flex-col relative">
        {/* Connector spine spans full row height, positioned to align
            with the icon column's left edge at the max-w-7xl gutter */}
        <div
          className="absolute top-2 bottom-2 w-px pointer-events-none hidden md:block"
          style={{
            left: "calc(50% - 40rem + 2rem)",
            background: "linear-gradient(to bottom, rgba(239,68,68,0.2), rgba(96,165,250,0.2), rgba(167,139,250,0.2))",
          }}
        />
        ...
        {AGENTS.map((agent, idx) => {
          ...
          return (
            <button
              ...
              className="group relative w-full text-left overflow-hidden border-b border-white/[0.06] first:border-t first:border-white/[0.06] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/30"
              style={{ "--scan-color": agent.iconColor } as React.CSSProperties}
            >
              {/* Full-bleed hover wash — extends edge to edge, not just the text column */}
              <div className={`absolute inset-0 transition-opacity duration-500 pointer-events-none bg-gradient-to-r ${agent.glow} via-transparent to-transparent ${isActive ? "opacity-100" : "opacity-0"}`} />
              <div className="agent-scanline" />

              {/* Inner content stays aligned to the same column as the heading */}
              <div className="max-w-7xl mx-auto px-4 md:px-8">
                <div className="grid grid-cols-[40px_1fr_40px] md:grid-cols-[52px_1fr_56px] gap-x-3 md:gap-x-6 py-5 md:py-8 items-center">
                  <span className="relative flex items-center gap-1.5 font-mono text-[11px] tracking-[0.12em] text-white/20 group-hover:text-white/45 transition-colors duration-300 select-none">
                  ...
                  <div className="relative flex flex-col min-w-0">
                    <span className={`text-xl sm:text-[clamp(20px,3vw,40px)] font-semibold tracking-[-0.03em] leading-none text-white/45 transition-colors duration-300 ${isActive ? "text-zinc-100" : ""}`}>{agent.name}</span>
                    <div
                      className="overflow-hidden transition-all duration-[380ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
                      style={{ maxHeight: isActive ? "160px" : "0px", opacity: isActive ? 1 : 0, marginTop: isActive ? "10px" : "0px" }}
                    >
                      <p className="text-[13px] leading-relaxed text-white/45 max-w-xl">{agent.desc}</p>
                  ...
                  <div className={`relative flex items-center justify-center w-9 h-9 md:w-11 md:h-11 rounded-[10px] transition-all duration-300 flex-shrink-0 justify-self-end ${isActive ? "scale-110" : "opacity-[0.14]"}`} style={{ background: agent.iconBg }}>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="mt-10 md:mt-14 grid grid-cols-3 rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.015)" }}>
          {[
...
// ─────────────────────────────────────────────
// FirebaseAuthButton
// Sits in the top-right corner of MainApp header.
// Shows nothing if Firebase isn't configured.
// Starts as anonymous, upgrades to Google on click.
// ─────────────────────────────────────────────

function FirebaseAuthButton({ user, loading }: { user: User | null; loading: boolean }) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // User dismissed popup — ignore
      if (e?.code !== "auth/popup-closed-by-user") {
        console.warn("[Phoenix] Google sign-in failed:", e?.message);
      }
    } finally {
      setSigningIn(false);
    }
  ...
  if (loading) return null; // Not yet initialised
    ...
    >
      {isAnon ? (
        <button
          ...
          disabled={signingIn}
          title="Sign in with Google to sync your session across devices"
          className="flex items-center gap-1.5 text-[10px] font-mono text-zinc-500 hover:text-zinc-300 border border-zinc-800 hover:border-zinc-600 bg-zinc-900/60 hover:bg-zinc-800/80 px-2 sm:px-2.5 py-1.5 rounded-lg transition-all disabled:opacity-50"
        >
          {/* Google G icon */}
          ...
          <span className="hidden sm:inline">{signingIn ? "Signing in…" : "Sign in to sync"}</span>
          <span className="sm:hidden">{signingIn ? "..." : "Sign In"}</span>
        </button>
      ) : (
        <div
          title={`Signed in as ${user!.displayName || user!.email || "Google user"} — session saved to Firestore`}
          className="flex items-center gap-1.5 text-[10px] font-mono text-emerald-500/80 border border-emerald-500/20 bg-emerald-950/20 px-2 sm:px-2.5 py-1.5 rounded-lg"
        >
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0" />
          <span className="max-w-[50px] sm:max-w-[100px] truncate">{user!.displayName?.split(" ")[0] || "Signed in"}</span>
          <span className="hidden sm:inline text-emerald-700">· ☁ Firebase</span>
        </div>
      )}
    </motion.div>
  );
...
export default function App() {
  ...
  // Initialise Firebase Auth: sign in anonymously so every visitor gets
  // a uid immediately. They can upgrade to Google Sign-In later.
  useLayoutEffect(() => {
    ...
    // Auto-sign in anonymously if no session exists yet
    if (!auth.currentUser) {
      signInAnonymously(auth).catch((e) => {
        console.warn("[Phoenix] Anonymous sign-in failed:", e?.message);
        setAuthLoading(false);
      });
    }
      ...
      <AnimatePresence mode="wait">
        {showLanding ? (
          ...
          <motion.div key="app" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.4, 0, 0.2, 1], delay: 0.08 }} onAnimationComplete={() => ScrollTrigger.refresh()}>
            <MainApp onBack={() => setShowLanding(true)} initialHours={initialHours} firebaseUser={firebaseUser} authLoading={authLoading} />
          </motion.div>
        )}
      </AnimatePresence>
    </ErrorBoundary>
...
function MainApp({ onBack, initialHours, firebaseUser, authLoading }: { onBack: () => void; initialHours?: number; firebaseUser: User | null; authLoading: boolean }) {
  ...
  const fireSpineFrom = (refPoint: React.RefObject<HTMLDivElement | null>) => {
  ...
  React.useEffect(() => {
    // Defensive: works whether sessionStore exports sync or async loadSession
    ...
    if (result && typeof (result as any).then === "function") {
      (result as any).then(handle).catch(() => { });
    } else {
      handle(result);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen text-zinc-100 selection:bg-red-500 selection:text-white" style={{ background: "#050507" }}>
      <OrbBg />
      <EmberFieldBackdrop
        urgency={
          intelligence
            ? { Critical: 1, High: 0.7, Moderate: 0.4, Low: 0.15 }[intelligence.risk_level] ?? 0.3
            : 0.3
        }
        particleCount={140}
      ...
      <div className="relative z-10">
        ...
        <header className="phoenix-header">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 sm:py-4 flex items-center justify-between gap-1.5 sm:gap-3 flex-nowrap min-w-0">
            <div className="flex items-center gap-1 sm:gap-2 min-w-0">
              <button onClick={onBack} className="text-[10px] sm:text-[11px] font-mono text-zinc-600 hover:text-zinc-400 transition-colors flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
                ← <span className="hidden sm:inline">Back</span>
              </button>
              {(intelligence || rescueResult) && (
                <>
                  <div className="w-px h-3 sm:h-4 bg-zinc-800 flex-shrink-0" />
                  <button type="button"
                    ...
                    className="text-[10px] sm:text-[11px] font-mono text-zinc-600 hover:text-zinc-400 transition-colors flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
                    <RotateCcw className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> <span className="hidden sm:inline">New</span>
                  </button>
                </>
              )}
              <div className="w-px h-3 sm:h-4 bg-zinc-800 flex-shrink-0" />
              <div className="p-1 sm:p-1.5 rounded-lg sm:rounded-xl shadow-lg flex-shrink-0" style={{ background: "linear-gradient(135deg, #EF4444, #F97316)", boxShadow: "0 0 16px rgba(239,68,68,0.4)" }}>
                <Flame className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
              </div>
              <h1 className="text-sm sm:text-lg font-bold flex-shrink-0" style={{ background: DS.fireGradient, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Phoenix</h1>
              <span className="hidden sm:inline text-[10px] font-mono px-2 py-0.5 rounded-full border truncate" style={{ borderColor: "rgba(239,68,68,0.25)", background: "rgba(239,68,68,0.06)", color: "rgba(239,68,68,0.8)" }}>Crisis Recovery Agent</span>
            </div>
            {/* Google Sign-In / Firebase status */}
            <FirebaseAuthButton user={firebaseUser} loading={authLoading} />
            <div className="text-right flex-shrink-0">
              {deadlineCountdown ? (
                <div>
                  <div className="flex items-center gap-1 sm:gap-1.5 justify-end">
                    <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-red-500 animate-ping flex-shrink-0" />
                    <span className="text-[10px] sm:text-[11px] font-mono font-bold text-red-400 tabular-nums">{deadlineCountdown}</span>
                  </div>
                  <div className="text-[8px] sm:text-[9px] font-mono text-zinc-700 uppercase tracking-wider mt-0.5"><span className="hidden sm:inline">until </span>deadline</div>
                </div>
              ) : (
        ...
        <AnimatePresence>
          {showRestoreBanner && restoredSession && (
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="max-w-7xl mx-auto px-4 md:px-8 mb-2 mt-2">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 pb-10">
          ...
          <section className="mb-8">
            <div className="flex items-center gap-1.5 text-zinc-600 text-[10px] font-mono uppercase tracking-[0.2em] mb-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {PRESETS.map((p, idx) => (
                <button key={idx} type="button" onClick={() => applyPreset(p)}
                  ...
                  style={{
                    background: (p as any).badge ? "rgba(239,68,68,0.04)" : "rgba(255,255,255,0.02)",
                    border: `1px solid ${(p as any).badge ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.06)"}`,
                  }}
                  
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.background = (p as any).badge ? "rgba(239,68,68,0.04)" : "rgba(255,255,255,0.02)";
                    (e.currentTarget as HTMLElement).style.borderColor = (p as any).badge ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.06)";
                    (e.currentTarget as HTMLElement).style.transform = "none";
                    (e.currentTarget as HTMLElement).style.boxShadow = "none";
                  }}
                >
                  {(p as any).badge && <span className="absolute top-2 right-2 text-[8px] font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", color: "#EF4444" }}>You're here</span>}
                  <h3 className="text-[10px] font-mono mb-1 uppercase tracking-wider" style={{ color: (p as any).badge ? "#EF4444" : "rgba(113,113,122,0.6)" }}>{p.title}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {/* Main Form */}
            <main className="phoenix-card lg:col-span-7 rounded-2xl p-6 relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-px" style={{ background: "linear-gradient(90deg, #EF4444, #F97316, transparent)" }} />
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <div className="relative">
                    <input type="text" required maxLength={140} value={goal} onChange={(e) => setGoal(e.target.value)}
                      ...
                      className="w-full text-sm text-zinc-100 placeholder-zinc-700 transition-all duration-200 phoenix-input"
                      style={{
                    ...
                    <div className="absolute right-12 top-1.5">
                      <VoiceInput mode="goal" onResult={(t) => setGoal(t)} disabled={loading} />
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" min="1" max="1000" required value={availableHours}
                      ...
                      className="w-full text-sm text-zinc-100 placeholder-zinc-700 transition-all duration-200 phoenix-input"
                      style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 14px", outline: "none" }} />
                  </div>
                  <div>
                    <label className="block text-[10px] font-mono uppercase tracking-[0.18em] text-zinc-500 mb-1.5 flex items-center gap-1">
                ...
                <div className="p-4 rounded-xl border transition-all duration-500" style={{ background: "rgba(255,255,255,0.02)", borderColor: progress > 0 ? `rgba(249,115,22,${0.08 + (progress / 100) * 0.2})` : "rgba(255,255,255,0.06)" }}>
                ...
                <div>
                  ...
                  <div onDragEnter={handleDrag} onDragOver={handleDrag} onDragLeave={handleDrag} onDrop={handleDrop}
                    ...
                    style={{
                      borderColor: pdfFile ? "rgba(52,211,153,0.3)" : dragActive ? "rgba(239,68,68,0.6)" : "rgba(255,255,255,0.08)",
                      background: pdfFile ? "rgba(52,211,153,0.04)" : dragActive ? "rgba(239,68,68,0.06)" : "rgba(255,255,255,0.02)",
                    }}>
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                      ...
                      <label className="block text-[10px] font-mono uppercase tracking-[0.18em] text-orange-400 mb-1.5 flex items-center gap-1.5">
                        ...
                        <div className="ml-auto">
                          <VoiceInput
                            mode="features"
                            onResult={(t) => setFeaturesText((prev) => prev ? prev + "\n" + t : t)}
                            disabled={loading}
                          />
                        </div>
                      </label>
                      <textarea rows={5} value={featuresText} onChange={(e) => setFeaturesText(e.target.value)}
                        ...
                        style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(249,115,22,0.15)", borderRadius: 10, padding: "10px 14px", outline: "none", resize: "none" }} />
                ...
                <button type="submit" disabled={loading || pdfReading}
                  className="w-full relative group overflow-hidden text-white font-semibold py-3.5 px-4 rounded-xl text-sm disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2 transition-all duration-200 hover:scale-[1.01] active:scale-[0.99]"
                  style={{ background: DS.fireBg, boxShadow: DS.fireShadow, letterSpacing: "0.04em" }}>
                  {loading ? (
            ...
            <aside className="lg:col-span-5 space-y-4">
              <div className="sidebar-card">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl border text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <div className="border p-4 rounded-xl text-left" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.06)" }}>
                    ...
                    {availableHours === "" || requiredHours === "" ? (
                      ...
                      <div>
                        <div className="flex items-baseline gap-1.5">
                          <span className={`text-4xl font-extrabold leading-none tracking-tight ${localDeficit > 0 ? "deficit-danger" : "deficit-safe"}`}>{localDeficit > 0 ? `+${localDeficit}` : localDeficit}</span>
              ...
              <div className="p-4 rounded-2xl flex items-start gap-2.5 text-left" style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)" }}>
          ...
          <AnimatePresence>
            {intelligence && (
              <motion.section ref={resultsSectionRef} initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 100, damping: 15 }}
                className="mt-14 sm:mt-20 pl-4 relative text-left"
              >
                <CrisisSpine ref={crisisSpineRef} height={crisisSpineHeight} />

                {/* Agent 1 — massive headline, not a card header */}
                <div ref={agent1RefPoint} className="pb-8 mb-8 border-b border-zinc-800/50">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[10px] font-mono text-zinc-600 uppercase tracking-[0.25em]">Agent 01 — Crisis Intelligence</span>
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-mono font-bold uppercase tracking-wider ${getRiskColors(intelligence.risk_level).bgColor} ${getRiskColors(intelligence.risk_level).borderColor} ${getRiskColors(intelligence.risk_level).textColor}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${getRiskColors(intelligence.risk_level).accentColor} animate-pulse`} />
                      {intelligence.risk_level} Risk
                    </div>
                  </div>
                  <h3 className="font-extrabold text-zinc-100 break-words leading-[0.95] tracking-tight" style={{ fontSize: "clamp(1.75rem, 5vw, 3.5rem)" }}>
                    "{goal}"
                  </h3>
                </div>

                {intelligence.note && (
                  <div className="flex items-start gap-2.5 bg-amber-950/25 border border-amber-500/20 text-amber-400 p-3.5 rounded-xl text-xs font-mono mb-6">
                ...
                {/* Metric row — borderless stats, not three boxed cards */}
                <div className="grid grid-cols-3 divide-x divide-zinc-800/50 mb-10 -mx-1">
                  {[
                    ...
                    <div key={i} className="px-3 sm:px-4 first:pl-1">
                      <span className="block text-[9px] font-mono text-zinc-600 uppercase tracking-wider mb-1.5">{m.label}</span>
                      <div className="flex items-baseline gap-1">
                        <span className={`text-2xl sm:text-4xl font-extrabold tabular-nums ${m.color}`}>{m.value}</span>
                ...
                {/* Failure causes — no box, just a left rule and big label */}
                <div className="pl-4 border-l-2 border-orange-500/40 relative mb-10">
                  <h4 className="text-[10px] font-mono uppercase tracking-[0.25em] text-orange-400 font-bold mb-4">Most Likely Failure Causes</h4>
                ...
                {/* ─── Agent 2+3 trigger — stage divider, not a card header ─────────────────── */}
                <div className="pt-10 mt-10 border-t border-zinc-800/50">
                  <div className="mb-6">
                    <span className="text-[10px] font-mono text-orange-400/70 uppercase tracking-[0.25em] block mb-1">Agents 02 + 03</span>
                    <h4 className="font-extrabold text-zinc-100 leading-[0.95] tracking-tight" style={{ fontSize: "clamp(1.5rem, 4vw, 2.75rem)" }}>Triage &amp; Rescue Plan</h4>
                    <p className="text-xs text-zinc-600 mt-2">Feature triage, then an hour-by-hour schedule, generated together.</p>
                  </div>
                  {!survivalResult && !triageLoading && (
                    <button type="button" onClick={handleGenerateTriageAndPlan}
                      className="w-full font-semibold py-3 px-4 rounded-xl transition-all text-sm flex items-center justify-center gap-2 active:scale-[0.99]"
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(244,244,245,0.9)" }}>
                ...
                <AnimatePresence>
                  {survivalResult && rescueResult && (
                    <motion.div
                      ...
                      initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="border-t border-zinc-800/60 pt-5 sm:pt-6 mt-5 sm:mt-6 space-y-5 sm:space-y-6">

                      {/* Triage header — large structural divider */}
                      <div>
                        <span className="text-[10px] font-mono text-emerald-400/70 uppercase tracking-[0.25em] block mb-1">Result</span>
                        <div className="flex items-baseline gap-3 flex-wrap">
                          <h4 className="font-extrabold text-zinc-100 leading-[0.95] tracking-tight" style={{ fontSize: "clamp(1.5rem, 4vw, 2.75rem)" }}>Survival Architecture</h4>
                          {triageSelfCorrected && (
                            <span
              ...
              <div ref={postPlanRef} className="space-y-5 sm:space-y-6">
                      ...
                      {/* ─── Agent 4 trigger — stage divider ─── */}
                      <div ref={agent4RefPoint} className="pt-10 mt-10 border-t border-zinc-800/50">
                        <div className="mb-6">
                          <span className="text-[10px] font-mono text-violet-400/70 uppercase tracking-[0.25em] block mb-1">Agent 04</span>
                          <h4 className="font-extrabold text-zinc-100 leading-[0.95] tracking-tight" style={{ fontSize: "clamp(1.5rem, 4vw, 2.75rem)" }}>Simulation Engine</h4>
                          <p className="text-xs text-zinc-600 mt-2">Your original plan vs. the Phoenix plan, simulated side by side.</p>
                        </div>
                        {!simResult && !simLoading && (
                          <button type="button" onClick={handleRunSimulation}
                            className="w-full font-semibold py-3 px-4 rounded-xl transition-all text-sm flex items-center justify-center gap-2 active:scale-[0.99]"
                            style={{ background: "rgba(167,139,250,0.07)", border: "1px solid rgba(167,139,250,0.2)", color: "rgba(244,244,245,0.9)" }}>
